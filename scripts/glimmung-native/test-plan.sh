...
#
# Feature types with a standing verification case skip this job entirely at
# the PLATFORM level: the workflow registration declares
# `when: "${{ vars.feature_type }} != 'effect'"` on the llm-test-plan job, so
# Glimmung never creates this job's pod for effect-type runs — the run graph
# renders the declared-but-skipped leg, and the verify wrapper sources the
# standing case from the repo itself. This script is the GENERATED path only.
#

...
- **Front plane** models rain crossing the viewer's screen/window plane. This
  ...
  frames, so they render as sparse, bright, short-lived streak flashes instead
  of tracked drops falling through the whole viewport. These are event-like
  streaks, not persistent streams: a drop that exits the viewport should not
  wrap back into the same screen-space path.

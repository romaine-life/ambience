	...
	function applyProceduralDefaults(kind, cfg) {
		...
		switch (kind) {
			...
			case 'mysterious-man':
				...
				if (c.figure_height <= 0) c.figure_height = base.figure_height;
				if (c.figure_width <= 0) c.figure_width = base.figure_width;
				c.silhouette = clamp01(c.silhouette);
				if (c.silhouette <= 0) c.silhouette = base.silhouette;
				c.hat = clamp01(c.hat);
				c.shoulder = clamp01(c.shoulder);
				if (c.ember_x <= 0) c.ember_x = base.ember_x;
				if (c.ember_y <= 0) c.ember_y = base.ember_y;
				if (c.ember_brightness <= 0) c.ember_brightness = base.ember_brightness;
				if (c.ember_pulse < 0) c.ember_pulse = 0;
				if (c.smoke_density < 0) c.smoke_density = 0;
				if (c.smoke_rise <= 0) c.smoke_rise = base.smoke_rise;
				if (c.smoke_softness <= 0) c.smoke_softness = base.smoke_softness;
				if (c.hue < 0) c.hue = base.hue;
				if (c.hue_sp < 0) c.hue_sp = 0;
				if (c.sat <= 0) c.sat = base.sat;
				...
				if (c.exhale_plume <= 0) c.exhale_plume = base.exhale_plume;
	...
	class ProceduralScene {
		...
		step() {
			...
			if (this.kind === 'mysterious-man') {
				if (!this.timers.inhale || this.timers.inhale <= 0) {
		...
		_renderMysteriousMan(ctx, canvasW, canvasH, opts) {
			...
			const reveal = this._revealLevelMysteriousMan();
			const ember = this._emberLevelMysteriousMan();

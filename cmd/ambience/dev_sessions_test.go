...
func TestNewDevSessionFirefliesSnapshotType(t *testing.T) {
	...
	snap, ok := session.snapshot().(map[string]any)
	if !ok {
		t.Fatalf("snapshot type = %T, want map[string]any", session.snapshot())
	}
	if got := snap["type"]; got != "fireflies" {
		t.Fatalf("snapshot type = %v, want fireflies", got)
	}
}
